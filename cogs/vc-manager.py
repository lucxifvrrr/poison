...
class VoiceManager(commands.Cog):
    ...
    async def check_bot_mute_permissions(
        self, ctx: commands.Context, channel: discord.abc.GuildChannel
    ) -> bool:
        bot_member = ctx.guild.get_member(ctx.bot.user.id)
        if not bot_member:
        ...
        bot_perms = channel.permissions_for(bot_member)
        if not bot_perms.mute_members:
            await ctx.send("<a:sukoon_reddot:1322894157794119732> I need Mute Members permission.")
            return False
        return True
    ...
    async def vcmute(self, ctx: commands.Context, confirm: str):
        ...
        vc = ctx.author.voice.channel
        if not await self.check_bot_mute_permissions(ctx, vc):
            return
