# skullboard.py
"""
Skullboard Cog - single-file, single slash command for full skullboard features.
...
- MONGO_URL   (required)
- DISCORD_TOKEN (for local quick tests if needed)

Usage:
Load as an extension in your bot:
    await bot.load_extension("cogs.skullboard")
"""
    ...
    def build_skull_embed(self, message:discord.Message, count:int, guild:discord.Guild, qualified:bool, cfg:SkullConfig) -> Embed:
        ...
        if atts:
            ...
            for idx, a in enumerate(atts[:MAX_ATTACHMENTS]):
                try:
                    if not image_set and a.content_type and a.content_type.startswith("image"):
